...
#test01-嘗試於part1加入multiprocess處理方式
'''
2017/4/22紀錄:在python3運行時會出現AttributeError: module 'wx' has no attribute 'OPEN'的問題
原因為python3安裝的是Phoenix版的wxpython, 而此計畫可能尚未成熟,所以會出現不預期錯誤
'''
import wx
import sys
import os
    ...
    def run(self):
                ...
                for h in range(0,len(sheetname2)):  #取得載入的EXCEL共有幾個Sheet
                        ...
                        if long(Reg.findall(sheetname2[h])[0]) in lis:  #須轉換為long型態才能跟sheetname2的頁面名稱匹配
                                ...
                                print(u"取得 %s 對應圖譜總表索引位置 %s" % (sheetname2[h],d))
