...
#bate1版本-增加計時功能
#choose版本:修改為輸入車規,僅針對輸入的車規進行分析,而不是全車規分析,可大幅度減短時間
import wx
import sys
import os
import openpyxl
...
class analysis_Thread(threading.Thread):
        ...
        def run(self):
                ...
                for lin in chooseEng:
                        for s in range(g,wb2.get_sheet_by_name(sheetname2[0]).max_column+1):
                                if wb2.get_sheet_by_name(sheetname2[0]).cell(row=4,column=s).value==lin:
                                        choosePart3Num.append(s)
                print(choosePart3Num)
