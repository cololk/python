...
#由填背景色改為變更字型顏色
#17/5/8紀錄:2-1版修正錯誤,欲分析欄位需每個sheet皆掃過,不能僅以第一個sheet作為代表,因為每個sheet的車規欄數可能會不同
#17/5/10紀錄:2-2版,新增判斷,如果該處應打"-"卻打成"O",或應打"O"卻打成"-"則以紅色顯示"-"
#17/5/17紀錄:2-3版,新增欲分析欄位(text5)判斷功能,可分辨是要分析一個範圍還是僅分析個別輸入的欄位(利用"~"進行區分),對應框架檔案frame3
#17/7/19紀錄:2-4版,改為可同時適用LC與GP, 主要變更點為將件號總數由寫死改為變數
import wx
import sys
import os
import openpyxl
...
Reg=re.compile(r'\d\d\d\d\d') #正規表示法,篩選同時有五個數字連續顯示的項目
...
class Myframe(frame3.MyFrame1):
        ...
        def openfile(self,event): #可開啟檔案對話框
                ...
                text5=self.m_textCtrl5.GetValue() #這行的目的是取得要分析的欄位,也可以設定在別處
...
class loadExcel_Thread(threading.Thread):
        ...
        def run(self): #要執行的程式碼定義在run()中,一旦開始此執行緒就會自動執行run()內容
                ...
                if sheetname[0] != u'圖譜總表':
                        ...
                        chooseNum=[]
                        chooseEng=[]
                        if "~" in text5: #判斷text5中是否有包含"~", 如果有就代表是要分析一個範圍
                            spt=text5.split("~")
                            for i in range(column_index_from_string(spt[0]), column_index_from_string(spt[1])+1):
                                chooseNum.append(i)
                                print(u"分析的車規為%s" % getsheet.cell(row=4, column=i).value)
                                chooseEng.append(getsheet.cell(row=4, column=i).value)
                        else:  #如果沒有"~",那就是個別分析輸入的欄位
                            choose=text5.split(',') 
                            for item in choose:
                        ...
                        print(u"開始建立件號組成資料...")
                        partNum=0
                        for i in range(11,getsheet.max_column+1): #此段主要目的為取得件號總數
                            if getsheet.cell(row=4,column=i).value is None:
                                pass
                            else:
                                res=Reg.findall(str(getsheet.cell(row=4, column=i).value))
                                if len(res)==1:
                                    partNum += 1
                        print(u"共有 %s 個件號" % partNum)
                        ...
                        def GereratePartContent(num):
                                ...
                                for j in range(5, getsheet.max_row+1): # j是圖譜的列數
                                                if getsheet.cell(row=j, column=getsheet.max_column-((partNum-1)-num)).value==u'\u25cf':
                                ...
                                return  getsheet.cell(row=4, column=getsheet.max_column-((partNum-1)-num)).value, PartContent
...
class analysis_Thread(threading.Thread):
        ...
        def run(self):
                ...
                for h in range(0,len(sheetname2)):  #取得載入的EXCEL共有幾個Sheet
                                ...
                                print("%s 分析完成".decode('utf8')% sheetname2[h])
                                choosePart3Num=[]
                                                                        ...
                                                                        for j in range(len(lis[d][get.cell(row=i, column=7).value])): #找出該件號該Spec_Code共出現幾個車規                   
                                                                                        for k in choosePart3Num:        
                                                                                                        ...
                                                                                                        if get.cell(row=4, column=k).value==lis[d][get.cell(row=i, column=7).value][j].lstrip('U:'):
                                                                                                            if lis[d][get.cell(row=i, column=7).value][j].startswith('U:'): #代表該位置應該是標記為"-"
                                                                                                                if get.cell(row=i, column=k).value is not None: #如果該欄位有值
                                                                                                                    if get.cell(row=i, column=k).value != u'\uff0d': #如果該爛位值不為"-",就標記為紅色(代表誤標為球點)
                                                                                                                        get.cell(row=i, column=k).value=u'\uff0d'
                                                                                                                        get.cell(row=i, column=k).font=Font(color="CE0000") #字型設定為紅色
                                                                                                                        get.cell(row=i, column=k).fill=Color_Red
                                                                                                                        print(u"[+][%s] %s 應為'-',誤標記為'O'" % (get.cell(row=4, column=k).value,get.cell(row=i, column=7).value ))
                                                                                                                    else:
                                                                                                                        get.cell(row=i, column=k).value=u'\uff0d'
                                                                                                                        get.cell(row=i, column=k).font=Font(color="006000") #字型為綠色
   
                                                                                                                else: #否則該欄位就是空,那就注入紅色(代表該選沒選)
                                                                                                                    get.cell(row=i, column=k).value=u'\uff0d'
                                                                                                                    get.cell(row=i, column=k).font=Font(color="CE0000") #字型設定為紅色
                                                                                                                    get.cell(row=i, column=k).fill=Color_Red
                                                                                                                    print(u"[+][%s] %s 漏標記'-'" % (get.cell(row=4, column=k).value,get.cell(row=i, column=7).value ))
                                                                                                            else:
                                                                                                                if get.cell(row=i, column=k).value is not None: #如果該欄位不是空(代表該選有選)
                                                                                                                    if get.cell(row=i, column=k).value != u'\u25cf': #如果該欄位不是"點"(代表誤打為"-")
                                                                                                                        get.cell(row=i, column=k).value=u'\u25cf'
                                                                                                                        get.cell(row=i, column=k).font=Font(color="CE0000") #字型設定為紅色
                                                                                                                        get.cell(row=i, column=k).fill=Color_Red
                                                                                                                        print(u"[+][%s] %s 應為'O',誤標記為'-'" % (get.cell(row=4, column=k).value,get.cell(row=i, column=7).value ))
                                                                                                                    else:
                                                                                                                        get.cell(row=i, column=k).value=u'\u25cf'
                                                                                                                        get.cell(row=i, column=k).font=Font(color="006000") #字型為綠色
                                                                                                                else: #否則該欄位就是空,那就注入紅色(代表該選沒選)
                                                                                                                                ...
                                                                                                                                get.cell(row=i, column=k).font=Font(color="CE0000") #字型設定為紅色
                                                                                                                                get.cell(row=i, column=k).fill=Color_Red
                                                                                                                                print(u"[+][%s] %s 該選未選"% (get.cell(row=4, column=k).value,get.cell(row=i, column=7).value ))                                                                                                               
