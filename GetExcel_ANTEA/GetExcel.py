...
class Myframe(baseFrame01.baseWind):
	def init_Myframe(self):
		self.m_textCtrl1.SetValue(Data_local+'LCH-DSHEET-170114.xlsx')
	...
	def LoadExcel(self,event):
		...
		print("創建Excel物件...載入CSCC".decode('utf-8'))

		for CsccName in CSCC_Name:
		    print("載入 %s".decode('utf-8') % (Data_local+CsccName))
	...
	def Search(self,event):
		...
		for attatch_no in object_no:
		    ...
		    for data in cscc_no:
		        if data==attatch_no:
		            print("廠商料號 %s 於CSCC中對應的原廠料號為 %s %s,其功能為%s".decode('utf-8') % (attatch_no, cscc_no[data][1],  cscc_no[data][0],cscc_no[data][2].strip()))        
		            flag=1        #找到資料,旗標就設定為1 
		            find=find+1
		            # 產生待比對附件編號對照料號表
		            ...
		            pass
		    if flag==0:
		        print("廠商料號 %s 無對應的原廠料號".decode('utf-8')% attatch_no)


		print("比對完畢".decode('utf-8'))
		print("共有 %s 筆資料, 比對出 %s 項".decode('utf-8') % (len(object_no),find))
		print(" ")
...
if __name__=='__main__':
	...
	main_win.init_redirectText() #展開重新導向
	main_win.Show()
	app.MainLoop()